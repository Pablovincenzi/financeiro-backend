    ...
    <section className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
      <article className="rounded-[1.75rem] border border-border bg-surface px-6 py-6">
        <div className="flex items-center justify-between gap-3">
          ...
          {pixEmEdicao ? <Link className="text-sm font-medium text-accent" href="/dashboard/pix">Cancelar edicao</Link> : null}
        </div>
      ...
      <article className="rounded-[1.75rem] border border-border bg-surface px-6 py-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold">Transacoes PIX</h2>
          </div>
          <span className="rounded-full bg-surface-strong px-3 py-1 text-sm text-muted">{transacoes.length} itens</span>
        </div>
        <div className="mt-6 space-y-3">
          {transacoes.length === 0 ? (
            <p className="rounded-2xl border border-dashed border-border px-4 py-6 text-sm text-muted">Nenhuma transacao PIX cadastrada ainda.</p>
          ) : (
            transacoes.map((pix) => (
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-col items-start gap-3 lg:items-end">
                    ...
                    <div className="flex gap-2">
                      <Link className="rounded-full border border-border px-3 py-2 text-sm font-medium transition hover:bg-surface-strong" href={`/dashboard/pix?edit=${pix.id}`}>Editar</Link>
