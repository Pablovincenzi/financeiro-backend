...
export default async function DashboardPage() {
  ...
  const [receitas, despesas, contasFixas, cartoes, compras, faturas] = await Promise.all([
  ...
  const saldoProjetado = totalReceitas - totalDespesas - totalContasFixas - totalCompras;
  ...
  return (
    <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
      ...
      <aside className="rounded-[1.75rem] border border-border bg-[#1f2937] px-6 py-6 text-white">
        <p className="text-sm uppercase tracking-[0.2em] text-white/70">Passo 5</p>
        <h2 className="mt-3 text-2xl font-semibold">Cartoes, compras e faturas integrados ao painel.</h2>
        <p className="mt-3 text-sm leading-7 text-white/75">
          O fluxo de cartao agora cobre cadastro do cartao, compras vinculadas e faturas por competencia. Parcelamentos continuam reservados para o proximo passo.
        </p>
        <div className="mt-6 grid gap-3">
          ...
          <Link className="rounded-2xl bg-accent px-4 py-3 text-center text-sm font-semibold text-white transition hover:bg-accent-strong" href="/dashboard/cartoes">
            Abrir cartoes
          </Link>
          <Link className="rounded-2xl border border-white/15 px-4 py-3 text-center text-sm font-semibold text-white transition hover:bg-white/8" href="/dashboard/compras-cartao">
            Abrir compras
          </Link>
          <Link className="rounded-2xl border border-white/15 px-4 py-3 text-center text-sm font-semibold text-white transition hover:bg-white/8" href="/dashboard/faturas">
            Abrir faturas
          </Link>
        </div>
      </aside>
