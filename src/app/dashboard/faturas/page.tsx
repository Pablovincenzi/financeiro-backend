...
export default async function FaturasPage({ searchParams }: PageProps) {
  ...
  const totalPago = faturas.reduce((sum, item) => sum + Number(item.valorPago), 0);
  const totalEmAberto = faturas.filter((item) => item.status !== "paga").reduce((sum, item) => sum + (Number(item.valorTotal) - Number(item.valorPago)), 0);
  ...
  return (
    <section className="grid gap-4">
      <DashboardPeriodHeader
        eyebrow="Faturas"
        title="Leia vencimentos e pagamentos de varias faturas dentro de um unico periodo."
        description={`Combine ${periodLabel} para somar vencimentos, valores pagos e saldo ainda em aberto nas faturas filtradas.`}
        pathname="/dashboard/faturas"
        selectedMonths={selectedMonths}
        monthOptions={monthOptions}
        extraParams={{ edit: params?.edit }}
        metrics={[
          { label: "Valor total", value: formatCurrency(totalFaturas), detail: "Soma das faturas listadas para os meses selecionados." },
          { label: "Valor pago", value: formatCurrency(totalPago), detail: "Montante ja quitado entre as faturas filtradas." },
          { label: "Em aberto", value: formatCurrency(totalEmAberto), detail: "Saldo restante nas faturas ainda nao pagas." },
        ]}
      />
      ...
      <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
        ...
        <DashboardListPanel title="Faturas cadastradas" totalLabel={formatCurrency(totalFaturas)}>
          {faturas.length === 0 ? (
            <p className="rounded-2xl border border-dashed border-border px-4 py-6 text-sm text-muted">Nenhuma fatura cadastrada para o periodo selecionado.</p>
          ) : (
            faturas.map((fatura) => (
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-col items-start gap-3 lg:items-end">
                    ...
                    <div className="flex gap-2">
                      <Link className="rounded-full border border-border px-3 py-2 text-sm font-medium transition hover:bg-surface-strong" href={`/dashboard/faturas?edit=${fatura.id}&months=${selectedMonths.join(",")}`}>Editar</Link>
